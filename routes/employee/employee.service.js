...
const getSingleEmployee = (req, res) => {
    Employee.findById(req.params.employee, {__v: 0}).populate('company', {
        name: 1,
        ...
        let d = new Date(employee.birthday), month = '' + (d.getMonth() + 1), day = '' + d.getDate(),
            year = d.getFullYear();
        if (month.length < 2)
            month = '0' + month;
        if (day.length < 2)
...
const createEmployee = (req, res) => {
    ...
    Employee.findOne({nationalNumber: req.body.nationalNumber.trim()}, (err, employee) => {
        ...
        if (err) return res.status(500).json({msg: "Server Error :)", err: console.log(err.message)});
        if (employee) return res.status(406).render('message', {message: "There is a employee with this national number."})
        Company.findOne({registrationNumber: req.body.company}, (err, company) => {
            if (err) return res.status(500).json({msg: "Server Error :)", err: console.log(err.message)});
            if (!company) return res.status(406).render('message', {message: "Company with this registration number doesn't exist."})

            if (req.body.beManager === 'yes') {
                Employee.findOne({company: company._id, beManager: true}, (err, employee) => {
                    if (err) return res.status(500).json({msg: "Server Error :)", err: console.log(err.message)});
                    if (employee) return res.status(406).render('message', {message: 'This Company already have a manager. if you want to change the manager, you must first change the manager info.'});
